...
- **iOS build**:
  ```
  DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -project ios/LinkyTracker.xcodeproj -scheme LinkyTracker -destination 'platform=iOS Simulator,name=iPhone 17,OS=26.2' build
  ```
- **Unit tests**: `bun test`
...
- Feature structure: `ios/LinkyTracker/Features/{Feature}/` with `atoms/`, `molecules/`, `organisms/`, `pages/` subdirectories
- Shared atoms: `ios/LinkyTracker/Shared/Components/` — cross-feature reusable views (badges, ratings, labeled rows)
...
- `.env` (`NITRO_API_TOKEN=...`)
- `ios/LinkyTracker/Shared/Secrets.swift` (gitignored)
- `ios/LinkyTrackerUITests/Support/TestSecrets.swift` (gitignored)
